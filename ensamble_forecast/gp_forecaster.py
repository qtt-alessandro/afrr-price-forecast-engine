...
def generate_gp_forecasts(gp_model, target_col, afrr_pr_ts_scl_train, afrr_pr_ts_scl_test, exog_ts_scl_train, exog_ts_scl_test, forecast_horizon, stride, afrr_pr_scaler):
    ...
    if target_col == 'aFRR_UpCapPriceEUR':
        gp_df_hat = gp_backtest_forecasts = concatenate(gp_backtest_forecasts).with_columns_renamed(
        ['aFRR_UpCapPriceEUR_cl_mu', 'aFRR_UpCapPriceEUR_cl_sigma'],
        col_names_new=['gp_afrr_up_cap_price_mu', 'gp_afrr_up_cap_price_sigma']).pd_dataframe()
        ...
        gp_df_hat = gp_backtest_forecasts = concatenate(gp_backtest_forecasts).with_columns_renamed(
        ['aFRR_DownCapPriceEUR_cl_mu', 'aFRR_DownCapPriceEUR_cl_sigma'],
        col_names_new=['gp_afrr_down_cap_price_mu', 'gp_afrr_down_cap_price_sigma']).pd_dataframe()
        
        gp_df_hat = pd.DataFrame(index=gp_backtest_forecasts.index)

        for q in quantiles:
            gp_df_hat[f'gp_afrr_down_cap_price_{q}'] = afrr_pr_scaler.inverse_transform(
                TimeSeries.from_series(
                    gp_backtest_forecasts['gp_afrr_down_cap_price_mu'] + 
                    stats.norm.ppf(q) * gp_backtest_forecasts['gp_afrr_down_cap_price_sigma']
                )
            ).pd_series()
...
def run_gp_pipeline(data_path, target_col, hyper_params_path, train_start, test_start, test_end, output_chunk_length, forecast_horizon, stride):
    ...
    ) = preprocess_afrr_data(data_path, train_start, test_start, test_end, val_start=None, use_validation=False, target_col=target_col)
